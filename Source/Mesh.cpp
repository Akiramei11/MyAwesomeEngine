...
void Mesh::LoadVBO(const tinygltf::Model& model, const tinygltf::Mesh& mesh, const tinygltf::Primitive& primitive)
{
	...
	float3* ptr1 = reinterpret_cast<float3*>(glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY));
	for (size_t i = 0; i < m_numVertices; ++i)
	{
		if (bufferPos != nullptr) {
			ptr1[i] = *reinterpret_cast<const float3*>(bufferPos);
			if (byteStride != 0) bufferPos += byteStride;
			else bufferPos += sizeof(float) * 3;
		}
		//if (bufferNorm != nullptr) {
		//	ptr1[i] = *reinterpret_cast<const float3*>(bufferNorm);
		//	if (byteStride != 0) bufferPos += byteStride;
		//	else bufferPos += sizeof(float) * 3;
		//}
		//if (bufferTex_0 != nullptr) {
		//	ptr1[i] = *reinterpret_cast<const float3*>(bufferTex_0);
		//	if (byteStride != 0) bufferPos += byteStride;
		//	else bufferPos += sizeof(float) * 2;
		//}
	}
...
void Mesh::Render(const std::vector<unsigned>& textures)
{
	...
	glUniform1i(glGetUniformLocation(program, "diffuse"), 0); // TODO:
	glBindVertexArray(m_vao);
